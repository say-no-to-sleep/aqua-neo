...
void main() {
  ...
  // center of shape 1
  vec2 p1 = (vec2(0, 0) - u_shape1Pos) / u_resolution.y;
  // center of shape 2
  ...
  if (STEP <= 0) {
    ...
    if (merged < 0.005) {
      float cssInside = -merged * u_resolution1x.y;
      float depth = clamp(cssInside / u_refThickness, 0.0, 1.0);
      float bevel = 1.0 - smoothstep(0.0, 1.0, depth);
      float x_R_ratio = 1.0 - cssInside / u_refThickness;
      float thetaI = safeAsin(pow(x_R_ratio, 2.0));
      float thetaT = safeAsin(1.0 / u_refFactor * sin(thetaI));
      float edgeFactor = -1.0 * tan(thetaT - thetaI);
      if (cssInside >= u_refThickness) edgeFactor = 0.0;

      vec2 surfaceNormal = getNormal3(p1, p2, gl_FragCoord.xy);
      vec2 sampleUv = v_uv;
      vec4 refracted;
      if (edgeFactor <= 0.0) {
      ...
      outColor = mix(refracted, vec4(smokeTint, 1.0), smokeOpacity);
      float vertical = abs(surfaceNormal.y);
      outColor.rgb = mix(
        outColor.rgb,
        vec3(0.025),
        bevel * pow(abs(surfaceNormal.x), 2.0) * 0.10
      );
      outColor.rgb = mix(outColor.rgb, vec3(1.0), bevel * pow(vertical, 2.0) * 0.10);
      outColor.rgb = mix(
        outColor.rgb,
        vec3(1.0),
    ...
    vec2 shadowP1 =
      (vec2(0.0) - u_shape1Pos + u_shadowPosition * u_dpr) / u_resolution.y;
    vec2 shadowP2 =
      (vec2(0.0) - u_mouseSpring + u_shadowPosition * u_dpr) / u_resolution.y;
    float shadowShape = mainSDF(shadowP1, shadowP2, gl_FragCoord.xy);
    float outside = smoothstep(-1.0 / u_resolution1x.y, 1.0 / u_resolution1x.y, merged);
    float shadowDistance = max(shadowShape, 0.0) * u_resolution1x.y;
    float contactRing = exp(-pow(shadowDistance / 14.0, 2.0)) * 0.11;
    float ambientHalo = exp(-pow(shadowDistance / u_shadowExpand, 2.0)) * 0.035;
    float shadow = (contactRing + ambientHalo) * outside * clamp(u_shadowFactor / 0.26, 0.0, 1.5);
    outColor.rgb = mix(outColor.rgb, vec3(0.04), clamp(shadow, 0.0, 0.16));
    float glassMask = 1.0 - smoothstep(-0.001, 0.001, merged);
    outColor.rgb = mix(outColor.rgb, vec3(1.0), 0.16 * u_pressed * glassMask);
    float outlineDistance = abs(merged) * u_resolution1x.y;
    float outline = 1.0 - smoothstep(0.0, 2.0, outlineDistance);
    vec2 outlineNormal = getNormal3(p1, p2, gl_FragCoord.xy);
    float outlineLight = pow(abs(outlineNormal.y), 2.0);
    outColor.rgb = mix(outColor.rgb, vec3(0.0), outline * mix(0.50, 0.26, outlineLight));
    outColor.rgb = mix(
      outColor.rgb,
      vec3(1.0),
      outline * pow(abs(outlineNormal.y), 12.0) * 0.14
    );
  }
